...
\section{Degree of a Map Induced by Divisors with Weighted Coefficients}
Let $C$ be a smooth projective curve over $\mathbb{C}$ of genus $4$, and view $C$ as a subvariety of its Jacobian $A := \operatorname{Jac}(C)$ via the Abel–Jacobi map. Consider the map
$$f:C\times C \times C \longrightarrow A \qquad (p_1,p_2,p_3) \longmapsto 2p_1+3p_2+4p_3.$$
What is the degree of $f$ onto its image? In other words, for a general point $a \in \operatorname{Im}(f)$, can we prove that $\# f^{-1}(a) = 1$?

This question is related to Brill–Noether theory, but I have not found an answer to this specific case, which concerns divisors with multiplicities. Insights from the tropical version might provide helpful ideas.

Thanks for your comment! However, I'm still quite confused. I don't know why there are two $g_3^1$'s. I don't understand how you compute the branch locus from the degeneracy class. Even if the map is unramified outside the singular locus of the normalization, it's not guaranteed that the degree of the cover is 1. Finally, can we find a counterexample for some specific $C$?
